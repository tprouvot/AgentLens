...
function normalizeTrace(json) {
  if (!json || typeof json !== 'object') return null;
  if (Array.isArray(json.plan)) return json;
  var steps = Array.isArray(json.allPlanSteps) && json.allPlanSteps.length
    ? json.allPlanSteps
    : Array.isArray(json.planSteps) ? json.planSteps : null;
  if (!steps) return null;
  return {
    plan: steps,
    planId: json.id || json.planId || null,
...
function loadData(json) {
  var normalized = normalizeTrace(json);
  if (!normalized) {
    showError('Unrecognized format. JSON must contain a "plan" or "allPlanSteps" array.');
    return;
  }
  if (normalized.plan.length > MAX_PLAN_STEPS) {
    showError('Plan exceeds maximum of ' + MAX_PLAN_STEPS + ' steps.');
    return;
  }
  currentData = normalized;
  selectedTopicId = null;
  var parsed = parsePlan(normalized.plan);
  ...
  meta.innerHTML = [
    normalized.planId ? '<span class="meta-chip"><strong>Plan</strong> ' + esc(String(normalized.planId).slice(0, 14)) + '…</span>' : '',
    normalized.sessionId ? '<span class="meta-chip"><strong>Session</strong> ' + esc(String(normalized.sessionId).slice(0, 14)) + '…</span>' : '',
